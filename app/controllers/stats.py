...
def get_stats_bp():
    ...
    return jsonify({'ok' : 'Stats Blueprint running'})

def jsonfier(prod):
	logger.debug('Constructing JSON response...')
	if not prod:
		if isinstance(prod,list):
			raise errors.AppError(10004, 'No products available with those filters!',200)
		raise errors.AppError(g.error['code'], g.error['message'])
	try:
		jp = jsonify(prod)
	except Exception as e:
		logger.error(e)
		raise errors.AppError(99999, 'Internal Error', 400)
